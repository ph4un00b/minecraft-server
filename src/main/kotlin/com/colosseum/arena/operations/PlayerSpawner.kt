...
class PlayerSpawner {
     ...
     */
    private fun calculateSpawnPoint(world: World, position: SpawnPosition, baseY: Int): SpawnPoint {
     ...
     */
    private fun buildSafetyFloor(world: World, spawnPoint: SpawnPoint, floorY: Int) {
        for (dx in -1..1) {
            for (dz in -1..1) {
    ...
    fun buildSpawnMarkers(world: World, baseY: Int) {
        SpawnPosition.getAll().forEach { position ->
            ...
            val markerMaterial = when (position) {
